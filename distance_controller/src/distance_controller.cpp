...
class DistanceControllerNode : public rclcpp::Node {
private:
  // member variables
  double px;
  double py;
...
public:
  // constructor
  DistanceControllerNode(std::vector<std::vector<float>> target_waypoints,
                         Control &pid_distance)
    ...
    this->subscriber_odom = this->create_subscription<nav_msgs::msg::Odometry>(
        ...
        std::bind(&DistanceControllerNode::subscriber_odom_callback, this,
                  std::placeholders::_1),
        sub_callback_g1);
    ...
    this->timer_control = this->create_wall_timer(
        ...
        std::bind(&DistanceControllerNode::timer_control_callback, this),
        callback_g2);
  }
};
...
int main(int argc, char *argv[]) {
  ...
  auto node = std::make_shared<DistanceControllerNode>(waypoints, pid_distance);
